...
//
// // Import Type
// import type { CommitOptions } from '../../types/git.types';
//
...
export async function manageCommits(targetRoot: string, options: CommitOptions = {}) {
	intro(pc.cyan('📝 Smart Commit Manager'));
	const s = spinner();
	...
	try {
		...
		if (status.isClean()) {
			outro('Working tree is clean. Nothing to commit.');
			return;
		}
		
		// 2. Stage Changes
		// If changes exist but nothing staged, ask to stage all
		if (status.staged.length === 0 && (status.modified.length > 0 || status.not_added.length > 0)) {
			const shouldStage = await confirm({
				message: 'No changes staged. Stage all changes?',
				initialValue: true
			});
		...
		if (!commitMessage) {
			...
			const availableAI = options.availableLLMs?.filter(p => p.available) || [];
			const canUseAI = availableAI.length > 0;
			
			let method = 'manual';
			
			if (canUseAI) {
				const choice = await select({
					message: 'Commit Message Strategy:',
					options: [
						{ value: 'ai', label: '🤖 AI Generated (Smart)' },
						{ value: 'manual', label: '✏️  Manual Entry' }
					]
				});
				if (isCancel(choice)) return;
				method = choice as string;
			}
			
			if (method === 'ai') {
				s.start('Analyzing diff...');
				// Get cached diff for context
				const diff = await git.diff(['--cached']);
				
				if (!diff) {
					s.stop('No diff content found.');
					logger.warn('Empty diff.');
					commitMessage = await text({ message: 'Enter commit message:' }) as string;
				} else {
					const safeDiff = llm.sanitizeContext(diff, 6000);
					const prompt = `
						Generate a semantic git commit message (Conventional Commits) for this diff.
						Format: <type>(<scope>): <subject>
						Return ONLY the message, no quotes or explanations.
					...
					try {
						const aiMsg = await llm.generate(prompt);
						s.stop('Message Generated.');
						
						// Review Cycle
						const confirmed = await text({
							message: 'Review Message (Enter to confirm, type to edit):',
							initialValue: aiMsg.trim()
						});
						
						if (isCancel(confirmed)) return;
						commitMessage = confirmed as string;
					} catch (e: any) {
				...
				if (isCancel(manual)) return;
				commitMessage = manual as string;
			}
		}
