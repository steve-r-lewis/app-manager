...
<#
 ...
 package.json, nuxt.config.ts, and README.md.
...
# Defaults
$searchPatterns = @("package.json", "nuxt.config.ts", "README.md")
...
if (-not $SkipMenu -and -not $Log -and -not $Debug) {
    ...
    $fileOptions = @("package.json", "nuxt.config.ts", "*.vue", "*.ts", "README.md")
...
try {
    ...
    $reportsBase = Join-Path $rootPath "app-monitor/reports"
    ...
    if (-not (Test-Path $reportsBase)) {
        ...
        Log-Debug "Created process reporting directory: $reportsBase"
    }
    ...
    $outputFile = Join-Path $reportsBase "monorepo-layer-descriptions-$timeStamp.md"
