...
This report is based on the **semantically complete pre-NCR command model** now represented on the post-PR #178 baseline. It deliberately describes commands in terms of **AppManager user intent**, rather than whichever Nuxt, Git, package-manager, filesystem, AI-provider, or other mechanism may ultimately perform the work.
