...
> **Status:** Active
>
> This document records the current implementation state of the AppManager documentation rationalisation work. It is not a design authority and must not redefine the Project Documentation Guide or the Design Specification.
...
This document belongs at the Implementation Specification level because it records the present state of the documentation system and the migration work required to align the repository with the approved documentation architecture.
...
A normative AI GitHub branch and Pull Request workflow has been proposed for inclusion in:

```text
docs/project-documentation-guide-v01.md
```

The proposed workflow requires AI systems to:
...
At the time this status document was created, that policy change is represented by:

```text
branch: ai/documentation-github-workflow
pull request: #6
```

Until Pull Request #6 is merged, the branch/PR policy is proposed rather than part of the authoritative `master` version of the Project Documentation Guide. It is nevertheless being followed for current AI-assisted repository changes.

---

## 14. Current Change for This Status Document

This status document is being introduced on a separate task branch:

```text
ai/documentation-rationalisation-status
```

The branch is intentionally separate from the AI workflow policy branch because the status document is a distinct documentation deliverable.
...
The Pull Request must be reviewed and merged by the user or another explicitly approved process.
...
1. review and merge or revise Pull Request #6 so that the AI GitHub workflow becomes authoritative;
2. review and merge or revise the Pull Request introducing this rationalisation status document;
3. correct the control character in `docs/appmanager-design-specification-v01.md` through a dedicated branch and Pull Request;
4. inventory and rationalise existing Functional Specifications, beginning with the existing command-domain documents;
5. transfer Functional material from archived sources and the reconciliation audit into authoritative Functional Specifications;
6. perform a second zero-information-loss reconciliation check for Functional material;
7. rationalise Detailed Design documentation by subsystem and command responsibility;
8. create or consolidate the root Implementation Specification and implementation-level supporting documents;
9. transfer remaining implementation observations from archived sources;
10. extract unresolved proposals into explicit proposal records where useful;
11. perform a final cross-level traceability review;
12. mark archived documents `-retired` only when their retirement criteria have actually been satisfied.
...
docs/appmanager-design-specification-v01.md
docs/design/appmanager-design-reconciliation-audit-v01.md
docs/implementation/documentation-rationalisation-status-v01.md
```
